...
class Parsing : MonoBehaviour
{
    DialogueEngine engine;
    EngineNode node;

    public TextAsset test_file;



    void Start()
    {
        //List<string> lines = File.ReadAllLines(Application.streamingAssetsPath + "\\Tree copy.txt").Select(line => line.Trim()).ToList();
        List<string> lines = test_file.text.Split("\n").Select(line => line.Trim()).ToList();
